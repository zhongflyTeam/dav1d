                                  ...
                                  const int have_fp)
{
    ...
    const int sign = msac_decode_bool_adapt(&t->msac, mv_comp->sign);
    const int cl = msac_decode_symbol_adapt(&t->msac, mv_comp->classes, 11);
    int up, fp, hp;
...
{
    switch (msac_decode_symbol_adapt(&t->msac, t->cdf.mv.joint, N_MV_JOINTS)) {
    case MV_JOINT_HV:
                         ...
                         const int x_off, const int y_off)
{
    ...
    if (depth < 2 && from > (int) TX_4X4) {
        ...
        is_split = msac_decode_bool_adapt(&t->msac, t->cdf.m.txpart[cat][a + l]);
        if (is_split)
                              ...
                              const int ref, uint64_t masks[2])
{
    ...
    int have_topright = imax(bw4, bh4) < 32 &&
                        have_top && t->bx + bw4 < t->tiling.col_end &&
...
{
    const Dav1dFrameContext *const f = t->f;
    const int pal_sz = b->pal_sz[pl] = 2 + msac_decode_symbol_adapt(&t->msac,
                                                 t->cdf.m.pal_sz[pl][sz_ctx], 7);
    uint16_t cache[16];
    int l_cache = pl ? t->pal_sz_uv[1][by4] : t->l.pal_sz[by4], n_cache = 0;
    // don't reuse above palette outside SB boundaries
    int a_cache = by4 & ~(!f->seq_hdr.sb128 << 4) ?
                  pl ? t->pal_sz_uv[0][bx4] : t->a->pal_sz[bx4] : 0;
    const uint16_t *l = t->al_pal[1][by4][pl], *a = t->al_pal[0][bx4][pl];

    // fill/sort cache
    while (l_cache && n_cache) {
        if (*l < *a) {
            if (!n_cache || cache[n_cache] != *l)
                cache[n_cache++] = *l++;
        } else {
            if (*a == *l) l++;
            if (!n_cache || cache[n_cache] != *a)
                cache[n_cache++] = *a++;
        }
    }
    if (l_cache) {
    ...
    for (int n = 0; n < n_cache && i < pal_sz; n++)
        if (msac_decode_bool(&t->msac, 128 << 7))
            cache[i++] = cache[n];
    n_cache = i;

    // parse new entries
    uint16_t *const pal = t->pal[pl];
    if (i < pal_sz) {
        int prev = pal[i++] = msac_decode_bools(&t->msac, f->cur.p.p.bpc);

        if (i < pal_sz) {
            int bits = f->cur.p.p.bpc - 3 + msac_decode_bools(&t->msac, 2);
            const int max = (1 << f->cur.p.p.bpc) - 1;

            do {
                const int delta = msac_decode_bools(&t->msac, bits);
        ...
        int n = 0, m = n_cache;
        for (i = 0; i < pal_sz; i++) {
            if (n < n_cache && (m >= pal_sz || cache[n] <= pal[m])) {
                pal[i] = cache[n++];
            } else {
                assert(m < pal_sz);
                pal[i] = pal[m++];
        ...
        memcpy(pal, cache, n_cache * sizeof(*cache));
    }
}
                        ...
                        const int sz_ctx, const int cbx4, const int cby4)
{
    ...
    // V pal coding
    const Dav1dFrameContext *const f = t->f;
    uint16_t *const pal = t->pal[2];
    if (msac_decode_bool(&t->msac, 128 << 7)) {
        const int bits = f->cur.p.p.bpc - 4 + msac_decode_bools(&t->msac, 2);
        int prev = pal[0] = msac_decode_bools(&t->msac, f->cur.p.p.bpc);
        const int max = (1 << f->cur.p.p.bpc) - 1;
        for (int i = 1; i < b->pal_sz[1]; i++) {
            int delta = msac_decode_bools(&t->msac, bits);
            if (delta && msac_decode_bool(&t->msac, 128 << 7)) delta = -delta;
            prev = pal[i] = (prev + delta) & max;
        }
    } else {
        for (int i = 0; i < b->pal_sz[1]; i++)
            pal[i] = msac_decode_bools(&t->msac, f->cur.p.p.bpc);
    }
}
                             ...
                             const int bw4, const int bh4)
{
    ...
    pal_idx[0] = msac_decode_uniform(&t->msac, b->pal_sz[pl]);
    uint16_t (*const color_map_cdf)[8 + 1] =
        t->cdf.m.color_map[pl][b->pal_sz[pl] - 2];
    for (int i = 1; i < 4 * (w4 + h4) - 1; i++) {
        ...
        for (int j = first, m = 0; j >= last; j--, m++) {
            const int color_idx =
                msac_decode_symbol_adapt(&t->msac, color_map_cdf[ctx[m]],
                                         b->pal_sz[pl]);
        ...
        b->max_ytx == TX_4X4)
    {
        ...
        if (DEBUG_BLOCK_INFO)
            printf("Post-vartxtree[%x/%x]: r=%d\n",
                   b->tx_split[0], b->tx_split[1], t->msac.rng);
        b->uvtx = av1_max_txfm_size_for_bs[bs][f->cur.p.p.layout];
    }
}
static void decode_b(Dav1dTileContext *const t, const enum BlockSize bs,
                     ...
                     const enum EdgeFlags intra_edge_flags)
{
    ...
    const int have_left = t->bx > t->tiling.col_start;
    const int have_top = t->by > t->tiling.row_start;
    const uint8_t *const b_dim = av1_block_dimensions[bs];
    const int bw4 = b_dim[0], bh4 = b_dim[1];
    ...
    if (f->frame_hdr.segmentation.enabled) {
        if (!f->frame_hdr.segmentation.update_map) {
                                                         ...
                                                         t->l.seg_pred[by4]])))
            {
                ...
                const unsigned diff = msac_decode_symbol_adapt(&t->msac,
                                                   t->cdf.m.seg_id[seg_ctx],
                                                   NUM_SEGMENTS);
    ...
    b->skip = b->skip_mode ? 1 :
              msac_decode_bool_adapt(&t->msac, t->cdf.m.skip[sctx]);
    if (DEBUG_BLOCK_INFO)
    ...
    if (!b->skip) {
        ...
        if (t->cur_sb_cdef_idx_ptr[idx] == -1) {
            const int v = msac_decode_bools(&t->msac, f->frame_hdr.cdef.n_bits);
            ...
            if (DEBUG_BLOCK_INFO)
                printf("Post-cdef_idx[%d]: r=%d\n",
                        *t->cur_sb_cdef_idx_ptr, t->msac.rng);
        }
    }
    ...
    {
        const int prev_qidx = t->last_qidx;
        const int have_delta_q = f->frame_hdr.delta_q_present &&
            (bs != (f->seq_hdr.sb128 ? BS_128x128 : BS_64x64) || !b->skip);
        if (have_delta_q) {
            int delta_q = msac_decode_symbol_adapt(&t->msac, t->cdf.m.delta_q, 4);
            if (delta_q == 3) {
                const int n_bits = 1 + msac_decode_bools(&t->msac, 3);
                delta_q = msac_decode_bools(&t->msac, n_bits) + 1 + (1 << n_bits);
            }
            if (delta_q) {
                if (msac_decode_bool(&t->msac, 128 << 7)) delta_q = -delta_q;
                delta_q *= 1 << f->frame_hdr.delta_q_res_log2;
            }
            t->last_qidx = iclip(t->last_qidx + delta_q, 1, 255);
            if (have_delta_q && DEBUG_BLOCK_INFO)
                printf("Post-delta_q[%d->%d]: r=%d\n",
                       delta_q, t->last_qidx, t->msac.rng);
        }
        if (t->last_qidx == f->frame_hdr.quant.yac) {
            // assign frame-wide q values to this sb
            t->dq = f->dq;
        } else if (t->last_qidx != prev_qidx) {
            // find sb-specific quant parameters
            init_quant_tables(&f->seq_hdr, &f->frame_hdr, t->last_qidx, t->dqmem);
            t->dq = t->dqmem;
        }

        // delta_lf
        int8_t prev_delta_lf[4];
        memcpy(prev_delta_lf, t->last_delta_lf, 4);
        if (have_delta_q && f->frame_hdr.delta_lf_present) {
            ...
            for (int i = 0; i < n_lfs; i++) {
                int delta_lf = msac_decode_symbol_adapt(&t->msac,
                                t->cdf.m.delta_lf[i + f->frame_hdr.delta_lf_multi], 4);
                if (delta_lf == 3) {
                    const int n_bits = 1 + msac_decode_bools(&t->msac, 3);
                    delta_lf = msac_decode_bools(&t->msac, n_bits) + 1 + (1 << n_bits);
                }
                if (delta_lf) {
                    if (msac_decode_bool(&t->msac, 128 << 7)) delta_lf = -delta_lf;
                    delta_lf *= 1 << f->frame_hdr.delta_lf_res_log2;
                }
                t->last_delta_lf[i] = iclip(t->last_delta_lf[i] + delta_lf, -63, 63);
                if (have_delta_q && DEBUG_BLOCK_INFO)
                    printf("Post-delta_lf[%d:%d]: r=%d\n", i, delta_lf, t->msac.rng);
            }
        }
        if (!memcmp(t->last_delta_lf, (int8_t[4]) { 0, 0, 0, 0 }, 4)) {
            // assign frame-wide lf values to this sb
            t->lflvl = f->lf.lvl;
        } else if (memcmp(t->last_delta_lf, prev_delta_lf, 4)) {
            // find sb-specific lf lvl parameters
            dav1d_calc_lf_values(t->lflvlmem, &f->frame_hdr, t->last_delta_lf);
            t->lflvl = t->lflvlmem;
        }
    }
    ...
    if (b->skip_mode) {
        ...
        const int ictx = get_intra_ctx(t->a, &t->l, by4, bx4,
                                       have_top, have_left);
        b->intra = !msac_decode_bool_adapt(&t->msac, t->cdf.m.intra[ictx]);
        if (DEBUG_BLOCK_INFO)
            printf("Post-intra[%d]: r=%d\n", b->intra, t->msac.rng);
    } else if (f->frame_hdr.allow_intrabc) {
        b->intra = !msac_decode_bool_adapt(&t->msac, t->cdf.m.intrabc);
        if (DEBUG_BLOCK_INFO)
    ...
    if (b->intra) {
        uint16_t *const ymode_cdf = f->frame_hdr.frame_type & 1 ?
            t->cdf.m.y_mode[av1_ymode_size_context[bs]] :
            t->cdf.kfym[intra_mode_context[t->a->mode[bx4]]]
                       [intra_mode_context[t->l.mode[by4]]];
        b->y_mode = msac_decode_symbol_adapt(&t->msac, ymode_cdf,
                                              N_INTRA_PRED_MODES);
        if (DEBUG_BLOCK_INFO)
        ...
        {
            uint16_t *const acdf = t->cdf.m.angle_delta[b->y_mode - VERT_PRED];
            const int angle = msac_decode_symbol_adapt(&t->msac, acdf, 7);
            b->y_angle = angle - 3;
        } else {
            b->y_angle = 0;
        }
        ...
        if (has_chroma) {
            ...
            uint16_t *const uvmode_cdf = t->cdf.m.uv_mode[cfl_allowed][b->y_mode];
            b->uv_mode = msac_decode_symbol_adapt(&t->msac, uvmode_cdf,
                                         N_UV_INTRA_PRED_MODES - !cfl_allowed);
            if (DEBUG_BLOCK_INFO)
            printf("Post-uvmode[%d]: r=%d\n", b->uv_mode, t->msac.rng);

            if (b->uv_mode == CFL_PRED) {
#define SIGN(a) (!!(a) + ((a) > 0))
                const int sign =
                    msac_decode_symbol_adapt(&t->msac, t->cdf.m.cfl_sign, 8) + 1;
                const int sign_u = sign * 0x56 >> 8, sign_v = sign - sign_u * 3;
                assert(sign_u == sign / 3);
                if (sign_u) {
                    const int ctx = (sign_u == 2) * 3 + sign_v;
                    b->cfl_alpha[0] = msac_decode_symbol_adapt(&t->msac,
                                            t->cdf.m.cfl_alpha[ctx], 16) + 1;
                ...
                if (sign_v) {
                    ...
                    b->cfl_alpha[1] = msac_decode_symbol_adapt(&t->msac,
                                            t->cdf.m.cfl_alpha[ctx], 16) + 1;
...
#undef SIGN
                if (DEBUG_BLOCK_INFO)
                    printf("Post-uvalphas[%d/%d]: r=%d\n",
                           b->cfl_alpha[0], b->cfl_alpha[1], t->msac.rng);
            } else if (b_dim[2] + b_dim[3] >= 2 && b->uv_mode >= VERT_PRED &&
                       b->uv_mode <= VERT_LEFT_PRED)
            {
                uint16_t *const acdf = t->cdf.m.angle_delta[b->uv_mode - VERT_PRED];
                const int angle = msac_decode_symbol_adapt(&t->msac, acdf, 7);
                b->uv_angle = angle - 3;
            } else {
                b->uv_angle = 0;
            ...
            imax(bw4, bh4) <= 16 && bw4 + bh4 >= 4)
        {
            ...
            if (b->y_mode == DC_PRED) {
                ...
                if (msac_decode_bool_adapt(&t->msac, t->cdf.m.pal_y[sz_ctx][pal_ctx]))
                    read_pal_plane(t, b, 0, sz_ctx, bx4, by4);
                if (DEBUG_BLOCK_INFO) {
                    printf("Post-y_pal[%d]: r=%d ", b->pal_sz[0], t->msac.rng);
                    for (int n = 0; n < b->pal_sz[0]; n++)
                        printf("%c%02x", n ? ' ' : '[', t->pal[0][n]);
                    printf("]\n");
                }
            }
            ...
            if (has_chroma && b->uv_mode == DC_PRED) {
                ...
                if (msac_decode_bool_adapt(&t->msac, t->cdf.m.pal_uv[pal_ctx]))
                    read_pal_uv(t, b, sz_ctx, cbx4, cby4);
                if (DEBUG_BLOCK_INFO) {
                    printf("Post-uv_pal[%d]: r=%d ", b->pal_sz[1], t->msac.rng);
                    for (int pl = 0; pl < 2; pl++) {
                        for (int n = 0; n < b->pal_sz[1]; n++)
                            printf("%c%02x", (n || pl) ? ' ' : '[', t->pal[1 + pl][n]);
                        printf("%c", pl ? ']' : ',');
                    }
                    printf("\n");
                }
        ...
        {
            const int is_filter = msac_decode_bool_adapt(&t->msac,
                                            t->cdf.m.use_filter_intra[bs]);
            if (is_filter) {
                b->y_mode = FILTER_PRED;
                b->y_angle = msac_decode_symbol_adapt(&t->msac,
                                                  t->cdf.m.filter_intra, 5);
            }
            if (DEBUG_BLOCK_INFO)
                printf("Post-filterintramode[%d/%d]: r=%d\n",
                       b->y_mode, b->y_angle, t->msac.rng);
        }

        uint8_t pal_idx[2][64 * 64];
        if (b->pal_sz[0]) {
            read_pal_indices(t, pal_idx[0], b, 0, w4, h4, bw4, bh4);
            if (DEBUG_BLOCK_INFO)
                printf("Post-y-pal-indices: r=%d\n", t->msac.rng);
        }

        if (has_chroma && b->pal_sz[1]) {
            read_pal_indices(t, pal_idx[1], b, 1, cw4, ch4, cbw4, cbh4);
            if (DEBUG_BLOCK_INFO)
        ...
        if (f->frame_hdr.segmentation.lossless[b->seg_id]) {
            ...
            if (f->frame_hdr.txfm_mode == TX_SWITCHABLE && t_dim->max > TX_4X4) {
                const int tctx = get_tx_ctx(t->a, &t->l, t_dim, by4, bx4,
                                            have_top, have_left);
                uint16_t *const tx_cdf = t->cdf.m.txsz[t_dim->max - 1][tctx];
                int depth = msac_decode_symbol_adapt(&t->msac, tx_cdf,
                                                     imin(t_dim->max + 1, 3));
        ...
        if (b->pal_sz[0]) {
            for (int x = 0; x < bw4; x++)
                memcpy(t->al_pal[0][bx4 + x][0], t->pal[0], 16);
            for (int y = 0; y < bh4; y++)
                memcpy(t->al_pal[1][by4 + y][0], t->pal[0], 16);
        }
        if (has_chroma) {
            ...
            if (b->pal_sz[1]) {
                for (int x = 0; x < cbw4; x++) {
                    memcpy(t->al_pal[0][cbx4 + x][1], t->pal[1], 16);
                    memcpy(t->al_pal[0][cbx4 + x][2], t->pal[2], 16);
                }
                for (int y = 0; y < cbh4; y++) {
                    memcpy(t->al_pal[1][cby4 + y][1], t->pal[1], 16);
                    memcpy(t->al_pal[1][cby4 + y][2], t->pal[2], 16);
                }
            }
        }
        ...
        av1_find_ref_mvs(mvstack, &n_mvs, mvlist, NULL,
                         (int[2]) { -1, -1 }, f->bw, f->bh,
                         bs, bp, t->by, t->bx, t->tiling.col_start,
                         t->tiling.col_end, t->tiling.row_start,
                         t->tiling.row_end, f->libaom_cm);
        ...
        if (mvlist[0][0].y | mvlist[0][0].x)
            ...
            if (t->by - (16 << f->seq_hdr.sb128) < t->tiling.row_start) {
                b->mv[0].y = 0;
        ...
        read_mv_residual(t, &b->mv[0], &t->cdf.dmv, 0);
        if (DEBUG_BLOCK_INFO)
            printf("Post-dmv[%d/%d,ref=%d/%d|%d/%d]: r=%d\n",
                   b->mv[0].y, b->mv[0].x, ref.y, ref.x,
                   mvlist[0][0].y, mvlist[0][0].x, t->msac.rng);
        ...
        if (b->skip_mode) {
            ...
            const int ctx = get_comp_ctx(t->a, &t->l, by4, bx4,
                                         have_top, have_left);
            is_comp = msac_decode_bool_adapt(&t->msac, t->cdf.m.comp[ctx]);
            if (DEBUG_BLOCK_INFO)
        ...
        if (b->skip_mode) {
            ...
            const int dir_ctx = get_comp_dir_ctx(t->a, &t->l, by4, bx4,
                                                 have_top, have_left);
            if (msac_decode_bool_adapt(&t->msac, t->cdf.m.comp_dir[dir_ctx])) {
                // bidir - first reference (fw)
                const int ctx1 = av1_get_fwd_ref_ctx(t->a, &t->l, by4, bx4,
                                                     have_top, have_left);
                if (msac_decode_bool_adapt(&t->msac,
                                           t->cdf.m.comp_fwd_ref[0][ctx1]))
                {
                    const int ctx2 = av1_get_fwd_ref_2_ctx(t->a, &t->l, by4, bx4,
                                                           have_top, have_left);
                    b->ref[0] = 2 + msac_decode_bool_adapt(&t->msac,
                                            t->cdf.m.comp_fwd_ref[2][ctx2]);
                } else {
                    const int ctx2 = av1_get_fwd_ref_1_ctx(t->a, &t->l, by4, bx4,
                                                           have_top, have_left);
                    b->ref[0] = msac_decode_bool_adapt(&t->msac,
                                            t->cdf.m.comp_fwd_ref[1][ctx2]);
                }
                ...
                const int ctx3 = av1_get_bwd_ref_ctx(t->a, &t->l, by4, bx4,
                                                     have_top, have_left);
                if (msac_decode_bool_adapt(&t->msac,
                                           t->cdf.m.comp_bwd_ref[0][ctx3]))
                {
                    b->ref[1] = 6;
                } else {
                    const int ctx4 = av1_get_bwd_ref_1_ctx(t->a, &t->l, by4, bx4,
                                                           have_top, have_left);
                    b->ref[1] = 4 + msac_decode_bool_adapt(&t->msac,
                                           t->cdf.m.comp_bwd_ref[1][ctx4]);
                }
            } else {
                // unidir
                                           ...
                                           t->cdf.m.comp_uni_ref[0][uctx_p]))
                {
                    ...
                    const int uctx_p1 = av1_get_uni_p1_ctx(t->a, &t->l, by4, bx4,
                                                           have_top, have_left);
                    b->ref[0] = 0;
                    b->ref[1] = 1 + msac_decode_bool_adapt(&t->msac,
                                           t->cdf.m.comp_uni_ref[1][uctx_p1]);
                    if (b->ref[1] == 2) {
                        const int uctx_p2 = av1_get_uni_p2_ctx(t->a, &t->l, by4, bx4,
                                                               have_top, have_left);
                        b->ref[1] += msac_decode_bool_adapt(&t->msac,
                                           t->cdf.m.comp_uni_ref[2][uctx_p2]);
                    }
                }
            }
            if (DEBUG_BLOCK_INFO)
                printf("Post-refs[%d/%d]: r=%d\n",
                       b->ref[0], b->ref[1], t->msac.rng);
            ...
            b->inter_mode = msac_decode_symbol_adapt(&t->msac,
                                             t->cdf.m.comp_inter_mode[ctx],
                                             N_COMP_INTER_PRED_MODES);
            if (DEBUG_BLOCK_INFO)
                printf("Post-compintermode[%d,ctx=%d]: r=%d\n",
                       b->inter_mode, ctx, t->msac.rng);
            ...
            if (b->inter_mode == NEWMV_NEWMV) {
                if (n_mvs > 1) {
                    ...
                    b->drl_idx += msac_decode_bool_adapt(&t->msac,
                                             t->cdf.m.drl_bit[drl_ctx_v1]);
                    if (b->drl_idx == 1 && n_mvs > 2) {
                        const int drl_ctx_v2 = get_drl_context(mvstack, 1);
                        b->drl_idx += msac_decode_bool_adapt(&t->msac,
                                             t->cdf.m.drl_bit[drl_ctx_v2]);
                    }
                    if (DEBUG_BLOCK_INFO)
                        printf("Post-drlidx[%d]: r=%d\n",
                               b->drl_idx, t->msac.rng);
                }
...
#define assign_comp_mv(idx, pfx) \
            ...
            case NEWMV: \
                b->mv[idx] = mvstack[b->drl_idx].pfx##_mv; \
                read_mv_residual(t, &b->mv[idx], &t->cdf.mv, 1); \
                break; \
            }
...
#undef assign_comp_mv
            ...
            if (f->seq_hdr.masked_compound) {
                ...
                is_segwedge = msac_decode_bool_adapt(&t->msac,
                                                 t->cdf.m.mask_comp[mask_ctx]);
                if (DEBUG_BLOCK_INFO)
                    printf("Post-segwedge_vs_jntavg[%d,ctx=%d]: r=%d\n",
                           is_segwedge, mask_ctx, t->msac.rng);
            }
            ...
            if (!is_segwedge) {
                if (f->seq_hdr.jnt_comp) {
                    ...
                    b->comp_type = COMP_INTER_WEIGHTED_AVG +
                        msac_decode_bool_adapt(&t->msac,
                                               t->cdf.m.jnt_comp[jnt_ctx]);
                    if (DEBUG_BLOCK_INFO)
                ...
                if (wedge_allowed_mask & (1 << bs)) {
                    ...
                    b->comp_type = COMP_INTER_WEDGE -
                        msac_decode_bool_adapt(&t->msac,
                                               t->cdf.m.wedge_comp[ctx]);
                    if (b->comp_type == COMP_INTER_WEDGE)
                        b->wedge_idx = msac_decode_symbol_adapt(&t->msac,
                                                t->cdf.m.wedge_idx[ctx], 16);
                } else {
                    b->comp_type = COMP_INTER_SEG;
                }
                b->mask_sign = msac_decode_bool(&t->msac, 128 << 7);
                if (DEBUG_BLOCK_INFO)
                    printf("Post-seg/wedge[%d,wedge_idx=%d,sign=%d]: r=%d\n",
                           b->comp_type == COMP_INTER_WEDGE,
                           b->wedge_idx, b->mask_sign, t->msac.rng);
            }
        } else {
            ...
            const int ctx1 = av1_get_ref_ctx(t->a, &t->l, by4, bx4,
                                             have_top, have_left);
            if (msac_decode_bool_adapt(&t->msac, t->cdf.m.ref[0][ctx1])) {
                const int ctx2 = av1_get_ref_2_ctx(t->a, &t->l, by4, bx4,
                                                   have_top, have_left);
                if (msac_decode_bool_adapt(&t->msac, t->cdf.m.ref[1][ctx2])) {
                    b->ref[0] = 6;
                } else {
                    const int ctx3 = av1_get_ref_6_ctx(t->a, &t->l, by4, bx4,
                                                       have_top, have_left);
                    b->ref[0] = 4 + msac_decode_bool_adapt(&t->msac,
                                                       t->cdf.m.ref[5][ctx3]);
                }
            } else {
                const int ctx2 = av1_get_ref_3_ctx(t->a, &t->l, by4, bx4,
                                                   have_top, have_left);
                if (msac_decode_bool_adapt(&t->msac, t->cdf.m.ref[2][ctx2])) {
            ...
            av1_find_ref_mvs(mvstack, &n_mvs, mvlist, &ctx,
                             ...
                             t->by, t->bx, t->tiling.col_start,
                             t->tiling.col_end, t->tiling.row_start,
                             t->tiling.row_end, f->libaom_cm);

            // mode parsing and mv derivation from ref_mvs
            if (msac_decode_bool_adapt(&t->msac, t->cdf.m.newmv_mode[ctx & 7])) {
                if (!msac_decode_bool_adapt(&t->msac,
                                        t->cdf.m.globalmv_mode[(ctx >> 3) & 1]))
                {
                ...
                if (DEBUG_BLOCK_INFO)
                    printf("Post-intermode[%d,drl=%d,mv=y:%d,x:%d]: r=%d\n",
                           b->inter_mode, b->drl_idx, b->mv[0].y, b->mv[0].x,
                           t->msac.rng);
            } else {
                has_subpel_filter = 1;
                b->inter_mode = NEWMV;
                ...
                if (DEBUG_BLOCK_INFO)
                    printf("Post-intermode[%d,drl=%d]: r=%d\n",
                           b->inter_mode, b->drl_idx, t->msac.rng);
                read_mv_residual(t, &b->mv[0], &t->cdf.mv, 1);
                if (DEBUG_BLOCK_INFO)
                    printf("Post-residualmv[mv=y:%d,x:%d]: r=%d\n",
                           b->mv[0].y, b->mv[0].x, t->msac.rng);
            }
            ...
            if (f->seq_hdr.inter_intra &&
                ...
                msac_decode_bool_adapt(&t->msac, t->cdf.m.interintra[ii_sz_grp]))
            {
                b->interintra_mode = msac_decode_symbol_adapt(&t->msac,
                                          t->cdf.m.interintra_mode[ii_sz_grp],
                                          N_INTER_INTRA_PRED_MODES);
                const int wedge_ctx = av1_wedge_ctx_lut[bs];
                b->interintra_type = INTER_INTRA_BLEND +
                    msac_decode_bool_adapt(&t->msac,
                                           t->cdf.m.interintra_wedge[wedge_ctx]);
                if (b->interintra_type == INTER_INTRA_WEDGE)
                    b->wedge_idx = msac_decode_symbol_adapt(&t->msac,
                                            t->cdf.m.wedge_idx[wedge_ctx], 16);
            } else {
                 ...
                 (have_top && findoddzero(&t->a->intra[bx4 + 1], w4 >> 1))))
            {
                ...
                uint64_t mask[2] = { 0, 0 };
                find_matching_ref(t, intra_edge_flags, bw4, bh4,
                ...
                b->motion_mode = allow_warp ?
                    msac_decode_symbol_adapt(&t->msac, t->cdf.m.motion_mode[bs], 3) :
                    msac_decode_bool_adapt(&t->msac, t->cdf.m.obmc[bs]);
                if (b->motion_mode == MM_WARP) {
                    has_subpel_filter = 0;
                    derive_warpmv(t, bw4, bh4, mask, b->mv[0], &b->warpmv);
#define signabs(v) v < 0 ? '-' : ' ', abs(v)
                    if (DEBUG_BLOCK_INFO)
                        printf("[ %c%x %c%x %c%x\n  %c%x %c%x %c%x ]\n"
                               "alpha=%c%x, beta=%c%x, gamma=%c%x, delta=%c%x\n",
                               signabs(b->warpmv.matrix[0]),
                               signabs(b->warpmv.matrix[1]),
                               signabs(b->warpmv.matrix[2]),
                               signabs(b->warpmv.matrix[3]),
                               signabs(b->warpmv.matrix[4]),
                               signabs(b->warpmv.matrix[5]),
                               signabs(b->warpmv.alpha),
                               signabs(b->warpmv.beta),
                               signabs(b->warpmv.gamma),
                               signabs(b->warpmv.delta));
#undef signabs
                }

                if (DEBUG_BLOCK_INFO)
                    printf("Post-motionmode[%d]: r=%d [mask: 0x%llx/0x%llx]\n",
                           b->motion_mode, t->msac.rng, mask[0], mask[1]);
            } else {
        ...
        if (f->frame_hdr.subpel_filter_mode == FILTER_SWITCHABLE) {
            if (has_subpel_filter) {
                ...
                const int ctx1 = get_filter_ctx(t->a, &t->l, comp, 0, b->ref[0],
                                                by4, bx4);
                b->filter[0] = msac_decode_symbol_adapt(&t->msac,
                    t->cdf.m.filter[0][ctx1], N_SWITCHABLE_FILTERS);
                if (f->seq_hdr.dual_filter) {
                    const int ctx2 = get_filter_ctx(t->a, &t->l, comp, 1,
                                                    b->ref[0], by4, bx4);
                    if (DEBUG_BLOCK_INFO)
                        printf("Post-subpel_filter1[%d,ctx=%d]: r=%d\n",
                               b->filter[0], ctx1, t->msac.rng);
                    b->filter[1] = msac_decode_symbol_adapt(&t->msac,
                        t->cdf.m.filter[1][ctx2], N_SWITCHABLE_FILTERS);
                    if (DEBUG_BLOCK_INFO)
                        printf("Post-subpel_filter2[%d,ctx=%d]: r=%d\n",
                               b->filter[1], ctx2, t->msac.rng);
                } else {
                ...
                b->filter[0] = b->filter[1] = FILTER_8TAP_REGULAR;
            }
        } else {
            b->filter[0] = b->filter[1] = f->frame_hdr.subpel_filter_mode;
        }
        ...
        const uint8_t (*const lf_lvls)[8][2] = (const uint8_t (*)[8][2])
            &t->lflvl[b->seg_id][0][b->ref[0] + 1][!is_globalmv];
        ...
        memset(&t->l.filter[0][by4], b->filter[0], bh4);
        memset(&t->a->filter[0][bx4], b->filter[0], bw4);
        memset(&t->l.filter[1][by4], b->filter[1], bh4);
        memset(&t->a->filter[1][bx4], b->filter[1], bw4);
                     ...
                     const EdgeNode *const node)
{
    ...
    if (0 && bl == BL_64X64)
        printf("poc=%d,y=%d,x=%d,bl=%d,r=%d\n",
               f->frame_hdr.frame_offset, t->by, t->bx, bl, t->msac.rng);
    const int bx8 = (t->bx & 31) >> 1, by8 = (t->by & 31) >> 1;
    const int ctx = get_partition_ctx(t->a, &t->l, bl, by8, bx8);
    uint16_t *const pc = t->cdf.m.partition[bl][ctx];
    enum BlockPartition bp;

    if (have_h_split && have_v_split) {
        const unsigned n_part = bl == BL_8X8 ? N_SUB8X8_PARTITIONS :
            bl == BL_128X128 ? N_PARTITIONS - 2 : N_PARTITIONS;
        bp = msac_decode_symbol_adapt(&t->msac, pc, n_part);
        ...
        switch (bp) {
        case PARTITION_NONE:
            decode_b(t, b[0], PARTITION_NONE, node->o);
            break;
        case PARTITION_H:
            decode_b(t, b[0], PARTITION_H, node->h[0]);
            t->by += hsz;
            decode_b(t, b[0], PARTITION_H, node->h[1]);
            t->by -= hsz;
            break;
        case PARTITION_V:
            decode_b(t, b[0], PARTITION_V, node->v[0]);
            t->bx += hsz;
            decode_b(t, b[0], PARTITION_V, node->v[1]);
            t->bx -= hsz;
            break;
        case PARTITION_SPLIT:
            if (bl == BL_8X8) {
                ...
                assert(hsz == 1);
                decode_b(t, BS_4x4, PARTITION_SPLIT, tip->split[0]);
                const enum Filter2d tl_filter = t->tl_4x4_filter;
                t->bx++;
                decode_b(t, BS_4x4, PARTITION_SPLIT, tip->split[1]);
                t->bx--;
                t->by++;
                decode_b(t, BS_4x4, PARTITION_SPLIT, tip->split[2]);
                t->bx++;
                t->tl_4x4_filter = tl_filter;
                decode_b(t, BS_4x4, PARTITION_SPLIT, tip->split[3]);
                t->bx--;
                t->by--;
            } else {
        ...
        case PARTITION_T_TOP_SPLIT: {
            const EdgeBranch *const branch = (const EdgeBranch *) node;
            decode_b(t, b[0], PARTITION_T_TOP_SPLIT, branch->tts[0]);
            t->bx += hsz;
            decode_b(t, b[0], PARTITION_T_TOP_SPLIT, branch->tts[1]);
            t->bx -= hsz;
            t->by += hsz;
            decode_b(t, b[1], PARTITION_T_TOP_SPLIT, branch->tts[2]);
            t->by -= hsz;
            break;
        }
        case PARTITION_T_BOTTOM_SPLIT: {
            const EdgeBranch *const branch = (const EdgeBranch *) node;
            decode_b(t, b[0], PARTITION_T_BOTTOM_SPLIT, branch->tbs[0]);
            t->by += hsz;
            decode_b(t, b[1], PARTITION_T_BOTTOM_SPLIT, branch->tbs[1]);
            t->bx += hsz;
            decode_b(t, b[1], PARTITION_T_BOTTOM_SPLIT, branch->tbs[2]);
            t->bx -= hsz;
            t->by -= hsz;
            break;
        }
        case PARTITION_T_LEFT_SPLIT: {
            const EdgeBranch *const branch = (const EdgeBranch *) node;
            decode_b(t, b[0], PARTITION_T_LEFT_SPLIT, branch->tls[0]);
            t->by += hsz;
            decode_b(t, b[0], PARTITION_T_LEFT_SPLIT, branch->tls[1]);
            t->by -= hsz;
            t->bx += hsz;
            decode_b(t, b[1], PARTITION_T_LEFT_SPLIT, branch->tls[2]);
            t->bx -= hsz;
            break;
        }
        case PARTITION_T_RIGHT_SPLIT: {
            const EdgeBranch *const branch = (const EdgeBranch *) node;
            decode_b(t, b[0], PARTITION_T_RIGHT_SPLIT, branch->trs[0]);
            t->bx += hsz;
            decode_b(t, b[1], PARTITION_T_RIGHT_SPLIT, branch->trs[1]);
            t->by += hsz;
            decode_b(t, b[1], PARTITION_T_RIGHT_SPLIT, branch->trs[2]);
            t->by -= hsz;
            t->bx -= hsz;
            break;
        }
        case PARTITION_H4: {
            const EdgeBranch *const branch = (const EdgeBranch *) node;
            decode_b(t, b[0], PARTITION_H4, branch->h4[0]);
            t->by += hsz >> 1;
            decode_b(t, b[0], PARTITION_H4, branch->h4[1]);
            t->by += hsz >> 1;
            decode_b(t, b[0], PARTITION_H4, branch->h4[2]);
            t->by += hsz >> 1;
            if (t->by < f->bh)
                decode_b(t, b[0], PARTITION_H4, branch->h4[3]);
            t->by -= hsz * 3 >> 1;
            break;
        }
        case PARTITION_V4: {
            const EdgeBranch *const branch = (const EdgeBranch *) node;
            decode_b(t, b[0], PARTITION_V4, branch->v4[0]);
            t->bx += hsz >> 1;
            decode_b(t, b[0], PARTITION_V4, branch->v4[1]);
            t->bx += hsz >> 1;
            decode_b(t, b[0], PARTITION_V4, branch->v4[2]);
            t->bx += hsz >> 1;
            if (t->bx < f->bw)
                decode_b(t, b[0], PARTITION_V4, branch->v4[3]);
            t->bx -= hsz * 3 >> 1;
            break;
        }
        default: assert(0);
        ...
        const unsigned is_split = msac_decode_bool(&t->msac, p);
        if (DEBUG_BLOCK_INFO)
            printf("poc=%d,y=%d,x=%d,bl=%d,ctx=%d,bp=%d: r=%d\n",
                   f->frame_hdr.frame_offset, t->by, t->bx, bl, ctx,
                   is_split ? PARTITION_SPLIT : PARTITION_H, t->msac.rng);
        ...
        if (is_split) {
            ...
            bp = PARTITION_H;
            decode_b(t, av1_block_sizes[bl][PARTITION_H][0], PARTITION_H,
                     node->h[0]);
        }
    } else {
        assert(have_v_split);
        ...
        if (DEBUG_BLOCK_INFO)
            printf("poc=%d,y=%d,x=%d,bl=%d,ctx=%d,bp=%d: r=%d\n",
                   f->frame_hdr.frame_offset, t->by, t->bx, bl, ctx,
                   is_split ? PARTITION_SPLIT : PARTITION_V, t->msac.rng);
        ...
        if (is_split) {
            ...
            bp = PARTITION_V;
            decode_b(t, av1_block_sizes[bl][PARTITION_V][0], PARTITION_V,
                     node->v[0]);
        }
    }
...
static int decode_tile(Dav1dTileContext *const t, const uint8_t *const data,
                       const size_t sz, const int tile_idx)
{
    const Dav1dFrameContext *const f = t->f;
    const Dav1dContext *const c = f->c;
    const int tile_row = tile_idx >> f->frame_hdr.tiling.log2_cols;
    const int tile_col = tile_idx & ((1 << f->frame_hdr.tiling.log2_cols) - 1);
    ...
    const int sb_shift = 4 + f->seq_hdr.sb128, sb_step = 1 << sb_shift;
    const int ss_ver = f->cur.p.p.layout == DAV1D_PIXEL_LAYOUT_I420;
    const int ss_hor = f->cur.p.p.layout != DAV1D_PIXEL_LAYOUT_I444;

    msac_init(&t->msac, data, sz);

    t->tiling.col_start = col_sb_start << sb_shift;
    t->tiling.col_end = imin(col_sb_end << sb_shift, f->bw);
    t->tiling.row_start = row_sb_start << sb_shift;
    t->tiling.row_end = imin(row_sb_end << sb_shift, f->bh);
    for (int sbx = col_sb128_start;
         sbx < ((col_sb_end + !f->seq_hdr.sb128) >> !f->seq_hdr.sb128); sbx++)
    {
        // FIXME tile boundaries can be at odd sb128 position, which
        // would break this since it acts on 128x128 units
        reset_context(&f->a[sbx], !(f->frame_hdr.frame_type & 1),
                      !row_sb_start);
    }

    // Reference Restoration Unit (used for exp coding)
    Av1RestorationUnit *lr_ref[3];
    Av1Filter *const lf_mask = f->lf.mask +
                               (t->tiling.row_start >> 5) * f->sb128w +
                               col_sb128_start;
    const int unit_idx = ((t->tiling.row_start & 16) >> 3) +
                         ((t->tiling.col_start & 16) >> 4);
    for (int p = 0; p < 3; p++) {
        lr_ref[p] = &lf_mask->lr[p][unit_idx];
        lr_ref[p]->filter_v[0] = 3;
        lr_ref[p]->filter_v[1] = -7;
        lr_ref[p]->filter_v[2] = 15;
        lr_ref[p]->filter_h[0] = 3;
        lr_ref[p]->filter_h[1] = -7;
        lr_ref[p]->filter_h[2] = 15;
        lr_ref[p]->sgr_weights[0] = -32;
        lr_ref[p]->sgr_weights[1] = 31;
    }

    const enum BlockLevel root_bl = f->seq_hdr.sb128 ? BL_128X128 : BL_64X64;
    const int f_bw[2] = { f->bw, f->bw >> ss_hor };
    const int f_bh[2] = { f->bh, f->bh >> ss_ver };
    for (t->by = t->tiling.row_start; t->by < t->tiling.row_end;
         t->by += sb_step)
    {
        // FIXME tile boundaries can be at odd sb128 position, which
        // would break this since it acts on 128x128 units
        reset_context(&t->l, !(f->frame_hdr.frame_type & 1), 1);
        memset(t->pal_sz_uv[1], 0, sizeof(*t->pal_sz_uv));
        const int sb128y = t->by >> 5;
        for (t->bx = t->tiling.col_start, t->a = f->a + col_sb128_start,
             t->lf_mask = f->lf.mask + sb128y * f->sb128w + col_sb128_start;
             t->bx < t->tiling.col_end; t->bx += sb_step)
        {
            ...
            for (int p = 0; p < 3; p++) {
                if (f->frame_hdr.restoration.type[p] == RESTORATION_NONE)
                    continue;
                const int by = !p ? t->by : t->by >> ss_ver;
                const int bx = !p ? t->bx : t->bx >> ss_hor;
                ...
                // restoration unit
                const int bottom_round = by && by + half_unit > f_bh[!!p];
                const int right_round = bx && bx + half_unit > f_bw[!!p];
                ...
                if (frame_type == RESTORATION_SWITCHABLE) {
                    const int filter =
                        msac_decode_symbol_adapt(&t->msac,
                                                 t->cdf.m.restore_switchable, 3);
                    ...
                    const unsigned type =
                        msac_decode_bool_adapt(&t->msac,
                                               frame_type == RESTORATION_WIENER ?
                                                   t->cdf.m.restore_wiener :
                                                   t->cdf.m.restore_sgrproj);
                    lr->type = type ? frame_type : RESTORATION_NONE;
                }

                if (lr->type == RESTORATION_WIENER) {
                    lr->filter_v[0] =
                        !p ? msac_decode_subexp(&t->msac,
                                                lr_ref[p]->filter_v[0] + 5, 16,
                                                1) - 5:
                             0;
                    lr->filter_v[1] =
                        msac_decode_subexp(&t->msac,
                                           lr_ref[p]->filter_v[1] + 23, 32,
                                           2) - 23;
                    lr->filter_v[2] =
                        msac_decode_subexp(&t->msac,
                                           lr_ref[p]->filter_v[2] + 17, 64,
                                           3) - 17;

                    lr->filter_h[0] =
                        !p ? msac_decode_subexp(&t->msac,
                                                lr_ref[p]->filter_h[0] + 5, 16,
                                                1) - 5:
                            0;
                    lr->filter_h[1] =
                        msac_decode_subexp(&t->msac,
                                           lr_ref[p]->filter_h[1] + 23, 32,
                                           2) - 23;
                    lr->filter_h[2] =
                        msac_decode_subexp(&t->msac,
                                           lr_ref[p]->filter_h[2] + 17, 64,
                                           3) - 17;
                    lr_ref[p] = lr;
                    if (DEBUG_BLOCK_INFO)
                        printf("Post-lr_wiener[pl=%d,v[%d,%d,%d],h[%d,%d,%d]]: r=%d\n",
                               ...
                               lr->filter_h[1], lr->filter_h[2], t->msac.rng);
                } else if (lr->type == RESTORATION_SGRPROJ) {
                    const unsigned idx = msac_decode_bools(&t->msac, 4);
                    lr->sgr_idx = idx;
                    lr->sgr_weights[0] = av1_sgr_params[idx][0] ?
                        msac_decode_subexp(&t->msac,
                                           lr_ref[p]->sgr_weights[0] + 96, 128,
                                           4) - 96 :
                        0;
                    lr->sgr_weights[1] = av1_sgr_params[idx][1] ?
                        msac_decode_subexp(&t->msac,
                                           lr_ref[p]->sgr_weights[1] + 32, 128,
                                           4) - 32 :
        ...
        // FIXME backup pre-loopfilter pixels for intra prediction of the next
        // sbrow
        //..
...
static int decode_frame(Dav1dFrameContext *const f) {
    const Dav1dContext *const c = f->c;
    ...
    if ((f->frame_hdr.frame_type & 1) || f->frame_hdr.allow_intrabc) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            f->refpoc[i] = c->refs[refidx].p.p.poc;
            if (c->refs[refidx].refmvs != NULL &&
                c->refs[refidx].p.p.p.w == f->cur.p.p.w &&
                c->refs[refidx].p.p.p.h == f->cur.p.p.h)
            {
                refmvs[i] = c->refs[refidx].refmvs->data;
            } else {
                refmvs[i] = NULL;
            }
            memcpy(refrefpoc[i], c->refs[refidx].refpoc,
                   sizeof(*refrefpoc));
        }
    ...
    if (f->frame_hdr.segmentation.enabled) {
        if (f->frame_hdr.segmentation.temporal) {
            const int pri_ref =
                f->frame_hdr.refidx[f->frame_hdr.primary_ref_frame];
            const int ref_w = (c->refs[pri_ref].p.p.p.w + 3) >> 2;
            const int ref_h = (c->refs[pri_ref].p.p.p.h + 3) >> 2;
            if (ref_w == f->bw && ref_h == f->bh) {
                f->prev_segmap_ref = c->refs[pri_ref].segmap;
    ...
    for (int i = 0; i < f->n_tile_data; i++) {
        ...
        for (int j = f->tile[i].start; j <= f->tile[i].end - empty_tiles; j++) {
            ...
            // FIXME tile threading - also interleave multiple tiles so
            // we decode a whole row per iteration here (or more, if possible)
            // and thus allow for interleaved frame/tile threading
            Dav1dTileContext *const t = f->tc;
            t->cdf = f->cdf;
            t->last_qidx = f->frame_hdr.quant.yac;
            memset(t->last_delta_lf, 0, sizeof(t->last_delta_lf));
            if (decode_tile(t, data, tile_sz, j)) goto error;
            data += tile_sz;
            size -= tile_sz;
    ...
    // cdf update
    if (update_set)
        av1_update_tile_cdf(&f->frame_hdr, &f->cdf, &update_cdf);
...
int submit_frame(Dav1dContext *const c) {
    // FIXME frame threading
    Dav1dFrameContext *f = c->fc;
    int res;
...
#define assign_bitdepth_case(bd) \
        ...
        f->bd_fn.filter_frame = filter_frame_##bd##bpc
    if (f->seq_hdr.bpc <= 8) {
#if CONFIG_8BPC
        assign_bitdepth_case(8);
#endif
...
#undef assign_bitdepth_case
    ...
    if (f->frame_hdr.primary_ref_frame == PRIMARY_REF_NONE) {
        av1_init_states(&f->cdf, f->frame_hdr.quant.yac);
    } else {
        const int pri_ref = f->frame_hdr.refidx[f->frame_hdr.primary_ref_frame];
        f->cdf = c->refs[pri_ref].cdf;
    }
    for (int i = 0; i < 8; i++)
