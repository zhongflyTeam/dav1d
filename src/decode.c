                           ...
                           const int bx4, const int by4)
{
    ...
    uint16_t *const pal = f->frame_thread.pass ?
        f->frame_thread.pal[(t->by >> 1) * (f->b4_stride >> 1) +
                            (t->bx >> 1)][pl] : t->pal[pl];
    if (i < pal_sz) {
...
#undef SIGN
            ...
            imax(bw4, bh4) <= 16 && bw4 + bh4 >= 4)
        {
            ...
            if (b->y_mode == DC_PRED) {
                ...
                if (msac_decode_bool_adapt(&ts->msac, ts->cdf.m.pal_y[sz_ctx][pal_ctx]))
                    read_pal_plane(t, b, 0, sz_ctx, bx4, by4);
                if (DEBUG_BLOCK_INFO) {
                    printf("Post-y_pal[%d]: r=%d ", b->pal_sz[0], ts->msac.rng);
            ...
            if (has_chroma && b->uv_mode == DC_PRED) {
                ...
                if (msac_decode_bool_adapt(&ts->msac, ts->cdf.m.pal_uv[pal_ctx]))
                    read_pal_uv(t, b, sz_ctx, cbx4, cby4);
                if (DEBUG_BLOCK_INFO) {
                    printf("Post-uv_pal[%d]: r=%d ", b->pal_sz[1], ts->msac.rng);
...
int decode_tile_sbrow(Dav1dTileContext *const t) {
    ...
    reset_context(&t->l, !(f->frame_hdr.frame_type & 1));
...
int decode_frame(Dav1dFrameContext *const f) {
    ...
    for (int n = 0; n < f->sb128w * f->frame_hdr.tiling.rows; n++)
        reset_context(&f->a[n], !(f->frame_hdr.frame_type & 1));
         ...
         f->frame_thread.pass <= 2 * uses_2pass; f->frame_thread.pass++)
    {
        ...
        if (f->n_tc == 1) {
            ...
            pthread_mutex_lock(&f->tile_thread.lock);
            assert(f->tile_thread.available == all_mask);
